...
class MyDetLocalVisualizer(Visualizer):
    ...
    def __init__(self,
                 ...
                 alpha: float = 0.8,
                 nms_iou_threshold: float = 0.3) -> None:
        super().__init__(
            name=name,
            image=image,
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'], # type: ignore
    ...
    def add_datasample(
        ...
        if draw_pred and data_sample is not None:
            ...
            if 'pred_instances' in data_sample:
                ...
                # bboxes = torch.concat([pred_instances.bboxes, pred_instances.scores.unsqueeze(-1)], dim=1)
                indices = nms(pred_instances.bboxes, pred_instances.scores, iou_threshold=self.nms_iou_threshold)
        ...
        # print(out_file)
        # data_sample.gt_instances
        if not is_bg and len(pred_instances) > len(data_sample.gt_instances):
            if out_file is not None:
                out_file = Path(out_file).parent.joinpath(os.path.basename(data_sample.img_path))
...
@VISUALIZERS.register_module()
class MyTrackLocalVisualizer(Visualizer):
